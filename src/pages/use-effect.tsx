...
const PageUseEffect = () => {
  ...
  return (
    <section className="flex flex-col gap-4 mb-12">
      ...
      <p>Estas son <strong>las tres fases que componen el ciclo de vida de un componente</strong>.</p>
    ...
    `}
      ...
      <Heading variant='h2' title="Otro ejemplo, pero esta vez contemplamos desmontarse" />
   ...
   return (
    ...
    `}
      ...
      <Heading variant='h2' title="Ahora vamos a ver un ejemplo de actualización con fetch" />

      <p>Vamos a entrar ya en faena a los casos más prácticos donde vamos a hacer un <strong>fetch</strong> a un endpoint <a target='_blank' rel='nofollow noopener' href="https://jsonplaceholder.typicode.com/users">(https://jsonplaceholder.typicode.com/users)</a> para luego guardar los datos en una variable <strong>data</strong>. Vemos que el estado inicial que hemos creado es <strong>null</strong> y que luego vamos a pasarle el resultado. De esta forma vamos a poder mostrar los nombres de la gente en pantalla. Por querer simplificar, no he querido hacer estados de <strong>loading</strong> o de <strong>error</strong>  por el momento.</p>
    ...
    `}
      ...
      <Heading variant='h2' title="Gestionando los estados de 'loading' y 'error'" />

      <p>Un caso muy típico con useEffect y fetch es cuando hacemos una llamada y tenemos que mostrarle al usuario los distintos estados: que estamos en mitad de la llamada, que ha salido bien o que ha salido mal. Para ello, podemos crear un par de variables más con useState y modificar el código.</p>
    ...
    `}
      ...
      <p>Pero qué pasa si queremos mostrar usuarios dependiendo de un filtro. Digamos que queremos filtrar a través de un input por nombre de usuario. Ahí podríamos hacer lo siguiente:</p>
  ...
  useEffect(() => {
    ...
    `}
      ...
      <p>Y como puedes comprobar, esta solución funciona, pero en mi opinión tenemos como mínimo dos problemas:</p>

      
      <ul className="list-disc pl-10">
        <li>
          Estamos haciendo un fetch cada vez que actualizamos una variable, lo cual no es muy eficiente. Sería mejor filtrar directamente de los datos que ya tenemos. Para ellos podríamos hacer uso del <Link href="/use-memo">useMemo</Link>.
        </li>
        <li>
          La variable filterName se actualiza cada vez que escribimos <strong>un caracter</strong>. Esto tampoco es eficiente. Para ello hay soluciones como decir que se actualice cada <strong>300ms</strong> después de que el usuario haya dejado de escribir. Esto lo podemos lograr con un <strong>Hook</strong> que se llama <Link href="/use-debounce">useDebounce</Link>.
        </li>
      </ul>
